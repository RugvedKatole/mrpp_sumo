...
class AN_MRPP:
    ...
    def callback_idle(self, data):
        if self.stamp < data.stamp:
            ...
            for i, n in enumerate(data.robot_id):
                ...
                #     writer.writerow(["time","edge","old node","current node","probability (value_exp)","denominator","expect","true","no.of neighbours"])
                    ...
                    if self.old_node[n] is not None :
                        ...
                        #if expect > true:
                        self.value_coeff[n][self.old_node[n]][self.current_node[n]] -= ((expect - true)/expect)
                        summ = 0
                        ...
                        for m in self.graph.successors(self.old_node[n]):
                            ...
                            writer.writerow([t,self.graph[self.old_node[n]][m]['name'],self.old_node[n],m,self.value_exp[n][self.old_node[n]][m],summ,self.idle_expect[n][self.old_node[n]][m],self.idle_true[m],len(list(self.graph.successors(self.old_node[n])))])
    ...
    def callback_next_task(self, req):
        ...
        #     writer = csv.writer(file)
        #     writer.writerow(['callback_next_task',bot,t,node,neigh[max_id]])
    ...
    def callback_ready(self, req):
        algo_name = req.algo
        if algo_name == 'observe_mrpp1' and self.ready:
