        ...
        if self.stamp < data.stamp:
            ...
            for i, n in enumerate(data.robot_id):
                with open('{}.csv'.format(n), 'a+', newline='') as file:
                    ...
                    writer.writerow(["time","edge","old node","current node","probability (value_exp)","value_coeff","sum","expect","true","no.of neighbours"])
                    ...
                    if self.old_node[n] is not None :
                        ...
                        for m in self.graph.successors(self.old_node[n]):
                            ...
                            writer.writerow([t,self.graph[self.old_node[n]][m]['name'],self.old_node[n],m,self.value_exp[n][self.old_node[n]][m],self.value_coeff[n][self.old_node[n]][m],summ,self.idle_expect[n][self.old_node[n]][m],self.idle_true[m],len(list(self.graph.successors(self.old_node[n])))])
