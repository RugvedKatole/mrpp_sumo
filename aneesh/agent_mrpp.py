...
class AN_MRPP:
        ...
        if self.stamp < data.stamp:
            ...
            for i,n in enumerate(data.robot_id):
                ...
                with open('{}.csv'.format(n), 'a+', newline='') as file:
                    ...
                    writer.writerow(["time","edge","probability (value_coeff)","expect","true","no.of neighbours"])

                    if self.old_node[n] is not None :
                        #if expect > true:
                        self.value_coeff[n][self.old_node[n]][self.current_node[n]] -= (0.2/np.log(t))*((expect - true)/expect)
                        for m in self.graph.successors(self.old_node[n]):
                            self.value_coeff[n][self.old_node[n]][m] += (0.2/np.log(t))*((expect - true)/expect)/(len(list(self.graph.successors(self.old_node[n]))))
                            writer.writerow([t,self.graph[self.old_node[n]][m]['name'],self.value_coeff[n][self.old_node[n]][m],self.idle_expect[n][m],self.idle_true[m],len(list(self.graph.successors(self.old_node[n])))])
    ...
    def callback_next_task(self, req):
        ...
        idles = []
        for n in neigh:
            idles.append((self.idle_expect[bot][n])*(self.value_coeff[bot][node][n]))
